...
This file is from tastytrade/tasty-client implementation
...
def conditional_color(value: Decimal, dollars: bool = True, percent: bool = False, round: bool = True) -> str:
    d = "$" if dollars else ""
    p = "%" if percent else ""
    if round:
        return (
            f"[red]-{d}{abs(value):.2f}{p}[/red]"
            if value < 0
            else f"[green]{d}{value:.2f}{p}[/green]"
        )
    return f"[red]-{d}{abs(value)}{p}[/red]" if value < 0 else f"[green]{d}{value}{p}[/green]"
